    ...
    start: function start (tag, attrs, unary) {
      ...
      function checkRootConstraints (el) {
        if ("development" !== 'production' && !warned) {
          if (el.tag === 'slot' || el.tag === 'templates') {
            warned = true;
            warn$2(
...
function processKey (el) {
  ...
  if (exp) {
    if ("development" !== 'production' && el.tag === 'templates') {
...
function genElement (el) {
  if (el.staticRoot && !el.staticProcessed) {
    ...
    return genIf(el)
  } else if (el.tag === 'templates' && !el.slotTarget) {
...
function genFor (el) {
  ...
  if (
    ...
    maybeComponent(el) && el.tag !== 'slot' && el.tag !== 'templates' && !el.key
  ) {
    warn$3(
...
function needsNormalization (el) {
  return el.for !== undefined || el.tag === 'templates' || el.tag === 'slot'
}
